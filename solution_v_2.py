...
def get_suitable_products(
        dealer_product: str, 
        manufactur_products: pd.DataFrame,
...
def get_solution(
    ...
    #КАК БУДЕТ ДОСТУП К СЕРВЕРАМ ДБ ТО ЗАМЕНИТЬ СТРИНГ В create_engine!!1
    engine = create_engine('11111111111111')
    manufactur_products = pd.read_sql('SELECT * FROM marketing_product', engine)
